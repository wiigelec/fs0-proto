...
def check_accepted_state_publication(root, assertion_ids):
    ...
    ok = not denied_missing['allowed'] and (not denied_rejected['allowed']) and allowed_create['allowed'] and (allowed_create['action'] == 'create') and allowed_noop['allowed'] and (allowed_noop['action'] == 'noop') and allowed_advance['allowed'] and (allowed_advance['action'] == 'advance') and ('decision = publication_decision(candidate, current, comments, module)' in source_text) and ('if not decision["allowed"]' in source_text) and ('git", "push", "origin"' in source_text) and ('accepted ref changed concurrently; refusing publication' in source_text)
    return [result(aid, 'pass' if ok else 'fail', 'accepted ref publication is denied without prior explicit acceptance and permitted only after a matching accepted candidate record exists') for aid in assertion_ids]
